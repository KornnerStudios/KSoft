...
namespace KSoft.IO
{
		...
		, IDisposable
	{
		...
		/// <remarks>
		/// <para>Length follows the rules of <see cref="EndianReader.ReadString(Memory.Strings.StringStorage, int)"/>. Fixed fields are consumed in full, including padding, even with a scan limit.</para>
		...
		/// <summary>Reads a string using a storage encoding.</summary>
		...
		/// <param name="maxLength">CString only: Optional maximum length of this specific string (exclusive of terminator)</param>
		/// <returns></returns>
		/// <seealso cref="ReadString(Text.StringStorageEncoding, int, int, int)"/>
		/// <seealso cref="Write(string, Text.StringStorageEncoding, int)"/>
