...
namespace KSoft.IO
{
	...
	static class JsonNodeGlobals
	{
		...
		public static bool HasErrorsOutput =>
			gErrorsOutputList != null && gErrorsOutputList.Count > 0;
	};
	...
	public struct JsonNode(IDictionary<string, object>? parsedData)
	{
		...
		public static JsonNode Null => new();

		public readonly IEnumerable<KeyValuePair<string, JsonNode>> ChildNodes { get {
			if (IsNull)
			{
				yield break;
			}
			...
			{
				if (kvp.Value is not IDictionary<string, object> nodeValues)
				{
					continue;
				}
		...
		public readonly IEnumerable<KeyValuePair<string, object>> RawData { get {
			if (IsNull)
			{
...
#pragma warning restore IDE0301 // Simplify collection initialization
		...
		public readonly ICollection<string> ChildNames { get {
			if (IsNull)
			{
		...
		public readonly string ToJson(bool prettyPrint = false)
		{
			return MiniJSON.Json.Serialize(mData, prettyPrint);
		}

		public readonly bool ContainsChild(string valueName)
		{
			if (IsNull)
			{
				return false;
		...
		public readonly JsonNode GetChild(string valueName)
		{
			if (IsNull)
			{
				return Null;
		...
		public readonly IEnumerable<JsonNode> GetArrayAsObjects(string valueName)
		{
			if (IsNull)
			{
				yield break;
			...
			{
				if (o is not IDictionary<string, object> arrayElement)
				{
					continue;
				}
		...
		public bool SetValuesForName(string valueName, IEnumerable<string> values)
		{
			...
			if (values is not List<string> list)
			{
...
#pragma warning restore IDE0306 // Simplify collection initialization
			...
			where TEnum : struct, Enum
		{
			...
			if (useArray)
			{
				var values = value.ToStrings(maxValue);
		...
		public readonly bool GetValue(string valueName, ref byte retVal)
		{
			int integer = 0;
		...
		public readonly bool GetValue(string valueName, ref float retVal)
		{
			...
			where TEnum : struct, Enum
		{
			...
			{
				case TypeCode.String:
					result = retVal.TryParseFlags<TEnum>((string)value, valueSeperator, logFailures ? JsonNodeGlobals.ErrorsOutputList : null);
					break;

				case TypeCode.Object:
				{
					if (value is not List<object> objList)
					{
						return false;
					}

					result = retVal.TryParseFlags<TEnum>(objList.OfType<string>(), logFailures ? JsonNodeGlobals.ErrorsOutputList : null);
					break;
				}
			...
			if (logFailures && JsonNodeGlobals.HasErrorsOutput)
			{
				Debug.Trace.IO.TraceDataSansId(System.Diagnostics.TraceEventType.Error, string.Format(Util.InvariantCultureInfo,
					"Failed parsing {0} value '{1}' as an {2}:\n{3}",
					valueName, value, "FLAGS", JsonNodeGlobals.ErrorsOutputList.Join("\n")));
				JsonNodeGlobals.ErrorsOutputList.Clear();
			}
