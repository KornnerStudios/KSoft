...
public sealed partial class PropertyChangedGenerator : IIncrementalGenerator
{
		...
		out PropertyModel? result)
	{
		...
		if (containingType.ContainingType != null || containingType.TypeKind != TypeKind.Class || candidate.Syntax.Parent is not ClassDeclarationSyntax)
		{
			ReportUnsupported(context, property, location, "only top-level classes are supported");
			return false;
		}
		...
		if (!TryReadChangedCallback(context, candidate, out string? changedCallback))
		{
			return false;
		}
		if (changedHook == ChangedHookMode.Parameterless && changedCallback != null)
		{
			ReportUnsupported(
				context,
				property,
				location,
				$"{GeneratorContracts.ChangedHookPropertyName} and {GeneratorContracts.ChangedCallbackPropertyName} cannot be combined");
			return false;
		}
	...
	private static string BuildSource(GeneratedType output)
	{
		...
		writer.WriteLine();
		if (output.Type.Arity != 0)
		{
			// Generic constraints can require class? or nullable constraint types.
		...
		writer.WriteLine(
			$"{GeneratedSourceUtilities.ModifiersText(declaration.Modifiers)}class {GeneratedSourceUtilities.EscapeIdentifier(output.Type.Name)}{TypeParameterListText(output.Type)}");
		foreach (string constraintClause in TypeParameterConstraintClauses(output.Type))
		{
	...
	private static string TypeParameterListText(INamedTypeSymbol type)
	{
		if (type.Arity == 0) return string.Empty;

		return "<" + string.Join(", ", type.TypeParameters.Select(
			static parameter => GeneratedSourceUtilities.EscapeIdentifier(parameter.Name))) + ">";
	}

	private static IEnumerable<string> TypeParameterConstraintClauses(INamedTypeSymbol type)
	{
		foreach (ITypeParameterSymbol parameter in type.TypeParameters)
		{
			...
			constraints.AddRange(parameter.ConstraintTypes.Select(
				static constraint => constraint.ToDisplayString(GeneratedSourceUtilities.ConstraintTypeDisplayFormat)));
			...
			{
				yield return
					$"where {GeneratedSourceUtilities.EscapeIdentifier(parameter.Name)} : {string.Join(", ", constraints)}";
			}
		}
	}
		...
		PropertyChangedHostStrategy strategy)
	{
		...
		// Generic partial declarations must repeat nullable type-parameter annotations to retain their signature.
		SymbolDisplayFormat typeDisplayFormat = model.ContainingType.Arity == 0
			? GeneratedSourceUtilities.TypeDisplayFormat
			: GeneratedSourceUtilities.ConstraintTypeDisplayFormat;
		string typeName = property.Type.ToDisplayString(typeDisplayFormat);
	...
	private sealed class PropertyModel
	{
		...
		public string? ChangedCallback { get; }
